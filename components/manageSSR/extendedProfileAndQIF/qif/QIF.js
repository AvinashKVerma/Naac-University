...
const QIF = () => {
  ...
  const handleSave = () => {
    const criteriaFunction =
      criteriaRef.current?.[`criteria${selectedCriteria + 1}Save`];

    // Call the selected criteria function if it exists
    criteriaFunction?.();
  };
        ...
        <p className="border border-slate-500 flex bg-white rounded-sm p-2 sticky top-[-24px] z-10 w-full justify-between">
          ...
          {selectedCriteria !== "" && (
            <span className="flex justify-end">
              <button
                onClick={handleSave}
                className="px-3 border border-[#329cf8] rounded-lg bg-[#329cf8]"
              >
                Save
              </button>
            </span>
